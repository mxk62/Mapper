...
def test_find_distance():
    ...
    # Identical, single-fragment patterns, expected distance 0.
    patt = Pattern('[C:1][O:2]')
    other = Pattern('[C:1][O:2]')
    assert abs(patt.find_distance(other) - 0.0) < 1.0e-06

    # Partially overlapping, single-fragment patterns, expected distance 0.5.
    patt = Pattern('[C:1][O:2]')
    other = Pattern('[C:1]')
    assert abs(patt.find_distance(other) - 0.5) < 1.0e-06

    # Non-overlapping, single-fragment patterns, expected distance 1.
    patt = Pattern('[C:1][O:2]')
    other = Pattern('[S:1]')
    assert abs(patt.find_distance(other) - 1.0) < 1.0e-06

    # Identical, multi-fragment patterns, expected distance 0.
    patt = Pattern('[C:4]O.[N:3][C:1]=[O:2]')
    other = Pattern('[C:4]O.[N:3][C:1]=[O:2]')
    assert abs(patt.find_distance(other) - 0.0) < 1.0e-06

    # Pair-wise, partially overlapping, multi-fragment patterns, expected
    # distance 0.5.
    patt = Pattern('[C:4]O.[N:3][C:1]=[O:2]')
    other = Pattern('[N:3].[C:2]O')
    assert abs(patt.find_distance(other) - 0.333333) < 1.0e-06

    # Partially overlapping, multi-fragment patterns, distance 1.
    patt = Pattern('[C:4]O.[N:3][C:1]=[O:2]')
    other = Pattern('[C:1]O.[S:2]')
    assert abs(patt.find_distance(other) - 1.0) < 1.0e-06

    # Pair-wise, non-overlapping, multi-fragment patterns, expected
    # distance 1.
    patt = Pattern('[C:4]O.[N:3][C:1]=[O:2]')
